...
static void HT_Yield_Thread(void *arg) {
    ...
    memset(&task_attr,0,sizeof(task_attr));
    memset(publicTaskStack, 0xA5,LED_TASK_STACK_SIZE);
    task_attr.name = "public_thread";
    task_attr.stack_mem = publicTaskStack;
    task_attr.stack_size = LED_TASK_STACK_SIZE;
    task_attr.priority = osPriorityNormal;
    task_attr.cb_mem = &public_thread;
    task_attr.cb_size = sizeof(StaticTask_t);

    osThreadNew(taskPubLdr, NULL, &task_attr);
